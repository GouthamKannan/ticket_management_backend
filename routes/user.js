...
router.post("/signup", async (req, res) => {
  ...
  const { user_name, email, password, role, user_type } = req.body;
  if (!user_name || !email || !password || !role || !user_type) {
  ...
  try {
    ...
    var sent = await userController.send_ver_link(user_name, email, hashOfPassword, role, user_type, req.protocol, req.get('host'));
    if (sent==true) {
      return res
      .status(200)
...
router.post("/login", async (req, res) => {
  ...
  try {
    ...
    if (userDetails.length == 1) {
        ...
        else if(userDetails[0].status == userController.account_status.inactive){
            return res.status(200).json({ success: false, data: "Email ID not verified"});
        }
        else if(userDetails[0].admin_ver == false){
          return res.status(200).json({ success: false, data: "Account not verified by admin"});
        }
        else {
        ...
        return res.status(200).json({ success: false, data: "Invalid user"});
      }
    }catch (error) {
...
router.get("/", async(req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ success: false, data: `Error in getting user ${error.message}` });
  }
})
...
router.post("/approve", async(req, res) => {
  try {
    ...
    // Get user details
    var response = await userController.getOneUserId(id)
    if(response.length==1)
    {
        // Change the account state to active
        await userController.approve_user(id)
    ...
    return res
      .status(500)
      .json({ success: false, data: `Error in approving user ${error.message}` });
  }
})
...
router.delete("/delete", async(req, res) => {
  try {
    ...
    // Get user details
    const response = await userController.getOneUserId(id)
    if(response.length==1)
    {
        // Change the account state to active
    ...
    return res
      .status(500)
      .json({ success: false, data: `Error in deleting user ${error.message}` });
  }
})
